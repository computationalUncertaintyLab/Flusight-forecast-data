...
hhs_sub <- hhs %>%
  dplyr::select(state, date, fips:total_patients_hospitalized_confirmed_influenza)
...
for (state_ in setdiff(unique(hhs_sub$state),c('DC'))){
  ...
  tmp_len <- length(hhs_sub[hhs_sub$state == state_ & hhs_sub$date > "2022-09-15",]$flu)
  current_season_flu <- hhs_sub[hhs_sub$state == state_ & hhs_sub$date > "2022-09-15",]$flu
