...
if __name__ == "__main__":
    ...
    for n,fil in enumerate(glob("./new_forecasts/*.csv")):
        d = pd.read_csv(fil)
        ...
        if n==0:
            d.to_csv("./new_forecasts/{:s}-{:s}.csv".format(monday_submission,model_name),index=False,header=True,mode="w")
        else: 
            d.to_csv("./new_forecasts/{:s}-{:s}.csv".format(monday_submission,model_name),index=False,header=False,mode="a")
