    ...
    for folder in os.listdir(root):
        ...
        if os.path.isdir(sub_dir):
            for fil in glob(sub_dir + "/*.csv"):
                ...
                MAE_scores = forecast_and_truths.groupby(["target","target_end_date","location","forecast_date"]).apply(lambda x: pd.Series({"MAE":median_absolute_error(x)}))
                MAE_scores = MAE_scores.reset_index()
                
                #--add model name
                model_name = forecast["model_name"].values[0]
                MAE_scores["model_name"] = model_name

                #--specify order of columns
                MAE_scores = MAE_scores[ ["model_name","target","target_end_date","location","forecast_date","MAE"]  ]

                #--write
                if n==0:
                    MAE_scores.to_csv("./retrospective_analysis/all_retrospective_mae_scores.csv", header=True ,index=False,mode="w")
                    n=1
                else:
                    MAE_scores.to_csv("./retrospective_analysis/all_retrospective_mae_scores.csv", header=False,index=False,mode="a")
